...
- 普通竖页：优先适配阅读区高度。
- 横向大图、双页阅读组：仍按阅读区高度适配，宽度可以超过阅读区。
- 保持比例。
- 不裁切。
- 默认允许放大和缩小。
...
进度条表达“当前阅读位置在阅读带里的视觉位置”。

这句话比“阅读进度百分比”更适合漫画阅读器。原因是漫画有阅读方向：

- LeftToRight：第一页在左侧，后续页向右展开。
- RightToLeft：第一页在右侧，后续页向左展开。

所以进度条不能只想成“页码越大，深色越长”。它应该和阅读空间保持一致。
...
- 深色区域表示当前阅读位置左侧。
- 浅色区域表示当前阅读位置右侧。
- RightToLeft 下，第一页在右侧，所以一开始进度条接近全深色；继续阅读时交界点向左移动，深色区域变短。
- LeftToRight 下，第一页在左侧，所以一开始进度条接近全浅色；继续阅读时交界点向右移动，深色区域变长。

当前 MVP 里，“当前阅读位置”等于“当前页”。

未来加入双页模式时，“当前阅读位置”可能不再等于单个页码。它可能表示：

- 当前双页组的位置。
- 双页中主读页的位置。
- 两页之间的中点。

所以文档里尽量使用“当前阅读位置”，而不是永远写死“当前页”。这样未来从单页扩展到双页、连续滚动、拖动预览时，不需要推翻这个概念。

这也是一个工程经验：

好的抽象不是把代码写复杂，而是给未来变化留一个准确的名字。
