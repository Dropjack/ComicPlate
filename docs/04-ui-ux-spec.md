...
- 默认使用 AutoFit：任何页面和阅读组都按阅读区高度尽量放大，保持“顶天立地”的阅读一致性；宽度可以超过阅读区，由横向阅读带、滚轮和拖拽处理。
- 邻近页面可以露出在当前阅读组两侧，也可以被阅读区边界裁掉；不能为了同时完整显示多张邻近页面、双页或宽页而把当前阅读组缩成小图。
- 图片保持比例，不裁切。
...
- 文件名或书名。
- 适配模式显示。
- 放大镜倍率状态显示在当前漫画信息区域最右侧。正常显示 `1.0x`；按住 `Z` 放大时显示当前倍率，例如 `1.8x`。
...
- 阅读：放大镜开关；说明 `Z - 放大`。该开关只控制 Reader 内部放大镜，不在 Command Rail 增加入口。
