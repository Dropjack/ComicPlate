...
ComicPlate 应该保留这个隐喻，因为它是 NeeView 体验舒服的根。
...
- 文件名使用自然排序。
- 只收集图片，不把子文件夹当页面。
- 递归模式开启时，收集子文件夹内图片，并按相对路径自然排序；不开启时只读当前文件夹。
...
- MVP 要有基础目录侧栏，用来承载 Book/Page 的核心阅读结构。
