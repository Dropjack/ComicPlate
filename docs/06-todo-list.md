...
- [x] 接入允许多开窗口设置。
- [ ] 添加文件关联设置面板的真实系统操作。
- [ ] 允许用户显式关联 CBZ、ZIP、CBR、RAR；默认不自动关联。
- [ ] 实现最小压缩包格式映射：ZIP/CBZ/RAR/CBR，不做通用 archive manager。
- [ ] 支持 RAR/CBR 打开、缩略图和进度恢复。
- [ ] 将 7z/CB7 移出当前 V1；不显示设置项，不预留可见占位。
- [ ] 确认嵌套压缩包不进入当前 V1，只忽略或按不可打开项处理。
- [ ] 实现快捷键预设或少量自定义。
- [x] 支持打开多个独立阅读器窗口。
- [x] 每个窗口独立打开一个 Book。
- [ ] 实现全屏。
- [ ] 全屏下工具栏自动隐藏。
- [ ] 优化吸附翻页动画：从自由 offset 或非吸附状态切到下一 frame 时，需要有平滑的 release/attack 手感。
- [ ] 优化阅读 frame 贴合：当前 frame、邻近 frame 和双页 frame 之间要严格贴合，避免出现破坏连续阅读感的空隙。
