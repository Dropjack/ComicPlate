...
- [x] 确认产品方向改为文件关联优先、启动面板、当前容器一层 Books+Pages 统一列表。
...
- [ ] 保存最近打开。
- [ ] 保存当前阅读组进度。
- [ ] exe 空启动展示 Continue / Open / Recent。
- [ ] 空启动 Continue 恢复上一次内容。
- [ ] 启动面板展示最近打开。
- [ ] 点击最近打开恢复阅读。
- [ ] 处理历史路径不存在。
- [ ] 写核心持久化测试。
...
- [ ] 实现 ZIP BookSource。
- [ ] 支持 `.zip` 和 `.cbz`。
- [ ] 支持 ZIP/CBZ 文件关联或命令行路径直接打开。
- [ ] 过滤压缩包内图片。
- [ ] 忽略非图片文件。
- [ ] 处理压缩包内子目录。
- [ ] 处理打不开的 ZIP。
- [ ] 处理加密 ZIP。
- [ ] 让 Continue/Recent 支持 ZIP/CBZ。
- [ ] 写 ZIP 排序和过滤测试。
...
- [ ] 文件夹打开后只扫描当前层。
- [ ] 当前层图片作为 Page entry。
- [ ] 当前层子文件夹作为 Book-like entry。
- [ ] 当前层 ZIP/CBZ 作为 Book-like entry。
- [ ] 当前层只有图片时直接作为文件夹漫画阅读。
- [ ] 当前层图片、文件夹、压缩包统一自然排序或按轻量分组排序。
- [ ] 统一列表不做后退栈、面包屑和树形文件浏览器。
- [ ] 处理空当前容器。
- [ ] 处理权限失败的子目录。
- [ ] 处理打不开的当前层压缩包。
- [ ] 写当前容器扫描测试。
